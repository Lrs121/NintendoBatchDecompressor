	...
	public class RLE
	{
		...
		public static byte[] Compress(Stream instream)
		{
			...
			while (readLength < inLength)
			{
				...
				{
					var flag = (byte) (numUncompToCopy - 1);
					compressedData.Add(flag);
					for (var i = 0; i < numUncompToCopy; i++)
						compressedData.Add(dataBlock[i]);
					// shift some possibly remaining bytes to the start
					for (var i = numUncompToCopy; i < currentBlockLength; i++)
			...
			{
				var flag = (byte) (currentBlockLength - 1);
				compressedData.Add(flag);
			...
			var compLen = compressedData.Count;
