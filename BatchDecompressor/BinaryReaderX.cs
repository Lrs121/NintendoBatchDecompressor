...
namespace BatchDecompressor
{
	public class BinaryReaderX : BinaryReader
	{
		...
		public BinaryReaderX(Stream input, Encoding encoding, ByteOrder byteOrder = ByteOrder.LittleEndian) : base(
			input, encoding)
		{
			ByteOrder = byteOrder;
		}

		public BinaryReaderX(Stream input, Encoding encoding, bool leaveOpen,
			ByteOrder byteOrder = ByteOrder.LittleEndian) : base(input, encoding, leaveOpen)
		{
			ByteOrder = byteOrder;
		}

		// Parameters out of order with a default encoding of Unicode
		public BinaryReaderX(Stream input, bool leaveOpen, ByteOrder byteOrder = ByteOrder.LittleEndian) : base(input,
			Encoding.Unicode, leaveOpen)
		{
			ByteOrder = byteOrder;
		}
		...
		{
			return string.Concat(Enumerable.Range(0, 999).Select(_ => (char) ReadByte()).TakeWhile(c => c != 0));
		}

		public string ReadCStringW()
		{
			return string.Concat(Enumerable.Range(0, 999).Select(_ => (char) ReadInt16()).TakeWhile(c => c != 0));
		}

		public string ReadCStringSJIS()
		{
			return Encoding.GetEncoding("Shift-JIS")
				.GetString(Enumerable.Range(0, 999).Select(_ => ReadByte()).TakeWhile(c => c != 0).ToArray());
		}

		public T ReadStruct<T>()
		{
			return ReadBytes(Marshal.SizeOf<T>()).BytesToStruct<T>(ByteOrder);
		}

		public List<T> ReadMultiple<T>(int count, Func<int, T> func)
		{
			return Enumerable.Range(0, count).Select(func).ToList();
		}

		public List<T> ReadMultiple<T>(int count)
		{
			return Enumerable.Range(0, count).Select(_ => ReadStruct<T>()).ToList();
		}
		...
		public byte SeekAlignment(byte alignmentByte, int alignment = 16)
		{
			return SeekAlignment(alignment, alignmentByte);
		}
	}
}