...
namespace BatchDecompressor
{
	...
	{
		// Read
		public static unsafe T BytesToStruct<T>(this byte[] buffer, ByteOrder byteOrder = ByteOrder.LittleEndian,
			int offset = 0)
		{
		...
		public static T BytesToStruct<T>(this byte[] buffer, int offset)
		{
			return BytesToStruct<T>(buffer, ByteOrder.LittleEndian, offset);
		}
