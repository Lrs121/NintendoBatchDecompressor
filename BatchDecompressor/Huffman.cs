...
namespace BatchDecompressor
{
	public class Huffman
	{
			...
			using (var bw = new BinaryWriterX(new MemoryStream()))
			{
				...
				foreach (var node in lst.Take(1).Concat(lst.SelectMany(node => node.children)))
				{
					if (node.children != null)
						node.code |= (byte) node.children
							.Select((child, i) => child.children == null ? (byte) (0x80 >> i) : 0).Sum();
					bw.Write(node.code);
				}
		...
		public class Node
		{
			...
			{
				return children?.SelectMany((child, i) => child.GetHuffCodes(seed + i)) ??
				       new[] {Tuple.Create(code, seed)};
			}
		}
	}
}